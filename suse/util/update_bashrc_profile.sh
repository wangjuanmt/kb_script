...
old="^.*env_vars.sh.*"
printf -v new ". \$HOME/bedroom/kb/script/suse/util/env_vars.sh"
grep -q '^.*env_vars.sh' $HOME/.profile && sed -i "s~$old~$new~" $HOME/.profile || echo -e ". \$HOME/bedroom/kb/script/suse/util/env_vars.sh\n$(cat $HOME/.profile)" > $HOME/.profile
