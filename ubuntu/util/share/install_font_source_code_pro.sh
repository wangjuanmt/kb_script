...
if [[ $DOWNLOAD_WITH_MIRROR == "help" ]]; then
    echo "Usage: install_font_source_code_pro.sh [download_with_mirror? (optional, 'true' or 'false', 'true' by default)]"
    exit 0
fi
...
FONT_TARGET_DIR=$USER_FONTS_DIR/SourceCodePro

# Downloading from mirror is recommended because it is always the latest version.
if [[ $DOWNLOAD_WITH_MIRROR == "true" ]]; then
    FONT_DOWNLOAD_MIRROR='https://mirrors.tuna.tsinghua.edu.cn/adobe-fonts/source-code-pro/OTF/'
...
# Downloading from official github repo requires version numbers.
    ...
    FONT_DOWNLOAD_TARBALL='https://github.com/adobe-fonts/source-code-pro/archive/2.030R-ro/1.050R-it.tar.gz'
    FONT_TAR_SAVE_NAME=SourceCodePro.tar.gz
    FONT_TAR_EXTRACTED_DIR_NAME=source-code-pro-*

    cd $USER_DOWNLOADS
    #wget $FONT_DOWNLOAD_TARBALL -O $FONT_TAR_SAVE_NAME
