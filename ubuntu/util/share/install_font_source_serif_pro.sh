...
if [[ $DOWNLOAD_WITH_MIRROR == "help" ]]; then
    echo "Usage: install_font_source_serif_pro.sh [download_with_mirror? (optional, 'true' or 'false', 'true' by default)]"
    exit 0
fi
...
FONT_TARGET_DIR=$USER_FONTS_DIR/SourceSerifPro

# Downloading from mirror is recommended because it is always the latest version.
if [[ $DOWNLOAD_WITH_MIRROR == "true" ]]; then
    FONT_DOWNLOAD_MIRROR='https://mirrors.tuna.tsinghua.edu.cn/adobe-fonts/source-serif-pro/OTF/'
...
# Downloading from official github repo requires version numbers.
    ...
    FONT_DOWNLOAD_TARBALL='https://github.com/adobe-fonts/source-serif-pro/archive/2.000R.tar.gz'
    FONT_TAR_SAVE_NAME=SourceSerifPro.tar.gz
    FONT_TAR_EXTRACTED_DIR_NAME=source-serif-pro-*
