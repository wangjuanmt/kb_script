...
FONT_TARGET_DIR=$USER_FONTS_DIR/source-han-sans

# Downloading from mirror is recommended because it is always the latest version.
if [[ $DOWNLOAD_WITH_MIRROR == "true" ]]; then
    FONT_DOWNLOAD_MIRROR='https://mirrors.tuna.tsinghua.edu.cn/adobe-fonts/source-han-sans/OTF/SimplifiedChinese/'
...
# Downloading from official github repo requires version numbers.
    ...
    FONT_DOWNLOAD_TARBALL='https://github.com/adobe-fonts/source-han-sans/archive/1.004R.zip'
    FONT_TAR_SAVE_NAME=SourceHanSans.tar.gz
    FONT_TAR_EXTRACTED_DIR_NAME=source-han-sans-*
    ...
    cp -rf ./OTF/SimplifiedChinese/*.otf $FONT_TARGET_DIR/
